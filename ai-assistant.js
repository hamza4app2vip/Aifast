...
analyzeSentimentBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (selectedAnalysisType === 'basic') {
            prompt = `قم بتحليل المشاعر في النص التالي وحدد ما إذا كان إيجابياً، سلبياً، أو محايداً. قدم نتيجة على شكل نسب مئائية للمشاعر الثلاثة مع شرح موجز. النص: ${text}`;
        } else if (selectedAnalysisType === 'detailed') {
            prompt = `قم بتحليل المشاعر في النص التالي بدقة وحدد المشاعر المحددة مثل السعادة، الحزن، الغضب، الخوف، المفاجأة، الاشمئزاز، وغيرها. قدم النتائج على شكل نسب مئوية لكل مشعر مع شرح مفصل. النص: ${text}`;
        } else { // emotional
            prompt = `قم بتحليل العواطف والانفعالات المعقدة في النص التالي. حدد المشاعر الأساسية والثانوية، وقدم تحليلاً عميقاً للدوافع العاطفية المحتملة. قدم النتائج على شكل نسب مئوية مع شرح مفصل. النص: ${text}`;
        }
        
        const response = await makeAPIRequest('chat/completions', {
            model: 'gpt-4o',
            messages: [
                { role: 'system', content: 'أنت محلل نفساني خبير في تحليل المشاعر والعواطف في النصوص. قدم تحليلات دقيقة ومفصلة باللغة العربية. يجب أن تكون إجاباتك بتنسيق JSON صالح.' },
                { role: 'user', content: prompt + '\n\nقدم النتائج بتنسيق JSON فقط، بدون أي نص إضافي. استخدم كلمة JSON في إجابتك.' }
            ],
